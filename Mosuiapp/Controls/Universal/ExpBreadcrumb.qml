...
radiusItemBg | [MosRadius](internal://MosRadius) | - | 路由项的背景圆角半径
\n<br/>
\n### 模型支持的属性：\n
属性名 | 类型 | 可选/必选 | 描述
------ | --- | :---: | ---
            ...
            code: `
                ...
                MosBreadcrumb {
                    ...
                    initModel: [
                        ...
                        { iconSource: MosIcon.UserOutlined, title: 'Application List' },
                        { loading: true, title: 'Application', },
                    ]
                }
            `
            exampleDelegate: MosBreadcrumb {
                initModel: [
                    { iconSource: MosIcon.HomeOutlined },
                    { iconSource: MosIcon.UserOutlined, title: 'Application List' },
                    { loading: true, title: 'Application', },
...
通过 \`model.separator\` 属性自定义单独的分隔符。
        ...
        CodeBox {
            ...
            desc: qsTr(`
面包屑支持下拉菜单。\n
通过 \`model.menu\` 属性设置为菜单即 [MosContextMenu](internal://MosContextMenu)。\n
通过 \`model.menu.items\` 属性设置菜单列表{即 \`MosContextMenu\` 的 \`initModel\` }。\n
